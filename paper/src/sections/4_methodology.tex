...
在每次训练时，我们随机选取时间步$t$，并对该时刻的噪声分布进行采样，作为E（n）等变的去噪内核的输入。该输入可以总体分为原子特征和其空间坐标。通过本文设计的双轨Transformer网络（Dual-track Transformer Network / DTN），我们即可得到网络预测样本在$t-1$时刻的分布。
