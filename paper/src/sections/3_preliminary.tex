...
随着时间步$t$的增加，噪声方差$\beta_t$从0平滑地过渡到1，这意味着向原始数据分布添加更多噪声。设$\bar{\alpha}_t = \prod^t_{s=1} \alpha_s = \prod^t_{s=1}(1-\beta_s)$，样本数据的分布可以表示为：
\begin{equation}
    q(G_t|G_0) = \mathcal{N}(G_t; \sqrt{\bar{\alpha}_t} G_0, (1 - \bar{\alpha}_t) I).
\end{equation} 

\subsubsection{去噪过程}
在去噪过程中，模型通过学习接近实际逆过程$q(G_{t-1} | G_t)$的马尔可夫核函数$p_\theta(G_{0:T-1}| G_{T}) = \prod^T_{t=1} p_\theta(G_{t-1} | G_t)$来重新构建原始的几何信息。每个时间步骤中学习到的参数化动力学的分布为： 
\begin{equation}
  p_\theta(G_{t-1} | G_t) = \mathcal{N}(G_{t-1}; \mu_\theta(G_t, t), \sigma_t^2 I),
\end{equation}
其中$\mu_\theta(G_t, t)$是用于近似均值的神经网络，$\sigma^2_t = \frac{(\beta_t - \beta_{t-1})\beta_{t-1}}{(1 - \beta_{t-1}) \beta_t}$是预定义的方差进度。初始时，$p_\theta(G_t)$从标准高斯分布中采样得到。然后通过迭代的逆过程逐步优化几何信息和原子特征。

理论上，训练目标采用数据对数似然的变分下界形式：
\begin{eqnarray}
...
\end{eqnarray}
然而，研究发现，预测高斯噪声$\epsilon$能够更容易进行神经网络训练。因此，$\mathcal{L}_t$ \cite{vaediff_kingma_21}的形式为
\begin{equation}
