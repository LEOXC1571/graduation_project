...
在更新坐标的方法上，我们遵循EDM \cite{edm_hoogeboom_22}和MDM \cite{mdm_huang_23}的相关设计。
\begin{equation}
...
\section{扩散及去噪过程}
扩散过程对于$t = 0, \ldots, T$ 需要定义 $\alpha_t, \sigma_t$。由于 $\alpha_t = \sqrt{1 - \sigma_t^2}$，只需定义 $\alpha_t$ 即可。这些值应该单调递减，从 $\alpha_0 \approx 1$ 开始，最终到达 $\alpha_T \approx 0$。在本文中，我们设定
\[
\alpha_t = (1 - 2s) \cdot f(t) + s \quad \text{其中} \quad f(t) = (1 - (t/T)^2)
\]
精度取 $10^{-5}$，以避免数值不稳定情况。这种计划在符号中与  中引入的余弦噪声计划非常相似，但我们的符号表述要简单一些。为了避免采样过程中的数值不稳定性，我们遵循 \citep{nichol2021improved} 的剪切过程，并计算 $\alpha_{t|t-1} = \alpha_t / \alpha_{t-1}$，其中我们定义 $\alpha_{-1} = 1$。然后，$\alpha_{t|t-1}^2$ 的值被剪切到下限 $0.001$。这样做是为了在采样过程中避免数值不稳定性，因为 $1 / \alpha_{t|t-1}$ 现在在采样过程中保持有界。然后，$\alpha_t$ 的值可以使用累积乘积 $\alpha_t = \prod_{\tau=0}^t \alpha_{\tau|\tau-1}$ 来重新计算。

回想一下，$\mathrm{SNR}(t) = \alpha_t^2 / \sigma_t^2$。就像  中一样，我们计算负对数 $\mathrm{SNR}$ 曲线定义为 $\gamma(t) = -(\log \alpha_t^2 - \log \sigma_t^2)$，其中 $\sigma_t^2 = 1 - \alpha_t^2$。$\gamma(t)$ 是一个单调递增的函数，可以用高数值精度计算出所有所需的分量。例如，$\alpha_t^2 = \mathrm{sigmoid}(-\gamma(t))$，$\sigma_t^2 = \mathrm{sigmoid}(\gamma(t))$，以及 $\mathrm{SNR}(t) = \exp(-\gamma(t))$。

在提出的GFMDiff的前向过程中，位置、原子序数和原子价逐渐被噪声先验分布所扰动。通过具有预定义方差序列$\beta_t \in (0, 1), t=0, 1, ..., T$的马尔可夫链将真实构象$G_0 = (P_0, X_0, A_0, V_0)$进行转换。后验分布定义为：
\begin{eqnarray}
